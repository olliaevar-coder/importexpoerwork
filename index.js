...
const { Client, Events, GatewayIntentBits, PermissionFlagsBits, ActionRowBuilder, StringSelectMenuBuilder, ButtonBuilder, ButtonStyle, REST, Routes } = require('discord.js');
...
const GUILD_ID = process.env.GUILD_ID || '1511120769063718912';
const COMMANDS = [{ name: 'order', description: 'Start a shipping order' }];
...
client.once(Events.ClientReady, async ready => {
  ...
  try {
    ...
    await rest.put(Routes.applicationGuildCommands(ready.user.id, GUILD_ID), { body: COMMANDS });
    console.log(`Registered /order in guild ${GUILD_ID}`);
  } catch (error) {
